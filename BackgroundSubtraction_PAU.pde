...
void setup() {
  ...
  antesPixels = new int[camPixels];
  // Cargar los pixeles del array para poder manipularlos
  loadPixels();
}
...
void draw() {
   if (video.available()) {
    ...
    for (int i = 0; i < camPixels; i++) { 
      ...
      color backgroundColor = antesPixels[i];
...
void keyPressed() {
  ...
  video.loadPixels();
  arraycopy(video.pixels, antesPixels);
  
    if (antesPixels != video.pixels) {
      fill(255,255,0);
